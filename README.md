<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Understand Domain" width="100%">
</p>
...
Technical work becomes difficult to review when discovery, decisions, changes, and verification are mixed together. **Understand Domain** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `understand-domain` names the capability directly—understand domain—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Understand Domain**.
...
| What does it do? | Extract business-domain knowledge from a codebase and generate an interactive domain flow graph. |
...
![Understand Domain system map](assets/system-map.svg)

## Operation sequence

![Understand Domain actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Understand Domain](assets/example-output.svg)
...
> Use a disposable sample repository to extract business-domain knowledge from a codebase and generate an interactive domain flow graph. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
