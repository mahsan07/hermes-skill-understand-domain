...
This package expresses **Understand Domain** as a typed, task-adaptive agent harness. Its concrete capability remains: Extract business-domain knowledge from a codebase and generate an interactive domain flow graph. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Understand Domain |
| --- | --- | --- |
| Memory | `entity_relationship_memory` | Retain repository evidence plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `dependency_and_path_traversal` | Transform domain entities into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through business rules while preserving stop conditions and user authority. |
